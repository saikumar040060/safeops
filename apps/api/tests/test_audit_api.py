...
def test_audit_filters_by_execution(client, seeded_db):
    ...
    response = client.get("/api/audit", params={"execution_id": start_a.execution_id})
...
def test_audit_filters_by_agent(client, seeded_db):
    ...
    response = client.get("/api/audit", params={"agent_id": str(support.id)})
    events = response.json()
    assert len(events) == 1
    assert events[0]["actor"] == "support-agent"


def test_audit_filters_by_event_type(client, seeded_db):
    agent = _agent(seeded_db, "support-agent")
    ...
    response = client.get("/api/audit", params={"event_type": "EXECUTION_STARTED"})
    events = response.json()
    assert len(events) == 2
    assert all(e["event_type"] == "EXECUTION_STARTED" for e in events)


def test_audit_pagination(client, seeded_db):
    agent = _agent(seeded_db, "support-agent")
    for i in range(5):
        runtime.start_execution(agent_id=agent.id, objective=f"obj-{i}", db=seeded_db)

    page1 = client.get("/api/audit", params={"limit": 2, "offset": 0}).json()
    page2 = client.get("/api/audit", params={"limit": 2, "offset": 2}).json()
    assert len(page1) == 2
    assert len(page2) == 2
    assert {e["id"] for e in page1}.isdisjoint({e["id"] for e in page2})


def test_audit_executions_endpoint_orders_by_sequence(client, seeded_db):
    agent = _agent(seeded_db, "support-agent")
    ...
    response = client.get(f"/api/audit/executions/{execution_id}")
    assert response.status_code == 200
    events = response.json()
