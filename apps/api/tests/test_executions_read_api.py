...
def test_execution_listing_filters_by_status(viewer_client, seeded_db):
    ...
    running = viewer_client.get("/api/executions", params={"status": "RUNNING"}).json()
    cancelled = viewer_client.get("/api/executions", params={"status": "CANCELLED"}).json()
...
def test_execution_listing_includes_current_step_and_risk(viewer_client, seeded_db):
    agent = _agent(seeded_db, "support-agent")
    ...
    body = viewer_client.get("/api/executions", params={"status": "RUNNING"}).json()
...
def test_execution_listing_pagination(viewer_client, seeded_db):
    agent = _agent(seeded_db, "support-agent")
    for i in range(5):
        runtime.start_execution(agent_id=agent.id, objective=f"obj-{i}", db=seeded_db)

    page1 = viewer_client.get("/api/executions", params={"limit": 2, "offset": 0}).json()
    page2 = viewer_client.get("/api/executions", params={"limit": 2, "offset": 2}).json()
    assert len(page1) == 2
    assert len(page2) == 2
    assert {e["id"] for e in page1}.isdisjoint({e["id"] for e in page2})


def test_execution_detail_returns_execution(viewer_client, seeded_db):
    agent = _agent(seeded_db, "support-agent")
    start = runtime.start_execution(agent_id=agent.id, objective="test objective", db=seeded_db)

    response = viewer_client.get(f"/api/executions/{start.execution_id}")
    assert response.status_code == 200
    assert response.json()["objective"] == "test objective"


def test_execution_detail_unknown_id_returns_404(viewer_client):
    response = viewer_client.get(f"/api/executions/{uuid.uuid4()}")
    assert response.status_code == 404


def test_execution_timeline_ordering_matches_audit_sequence(viewer_client, seeded_db):
    agent = _agent(seeded_db, "support-agent")
    start = runtime.start_execution(
        agent_id=agent.id,
        objective="Investigate duplicate payment for CUST-1001 and refund the duplicate",
    ...
    response = viewer_client.get(f"/api/executions/{execution_id}/timeline")
    assert response.status_code == 200
    body = response.json()
...
def test_execution_timeline_unknown_id_returns_404(viewer_client):
    response = viewer_client.get(f"/api/executions/{uuid.uuid4()}/timeline")
    assert response.status_code == 404


def test_execution_timeline_audit_events_do_not_leak_raw_malicious_prompt(viewer_client, seeded_db):
